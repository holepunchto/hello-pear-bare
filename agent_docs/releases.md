...
- The applied binary takes effect on the **next** run — the daemon applies after
  the foreground has already exited, so a user may need two runs to see a new
  version. `updates.log` records what the daemon did.
